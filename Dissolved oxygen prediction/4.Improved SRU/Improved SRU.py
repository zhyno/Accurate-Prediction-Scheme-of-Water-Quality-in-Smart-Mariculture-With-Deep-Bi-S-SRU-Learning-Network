...
import pandas as pd
import time
from Improved_sruM import Improved_SRUCell
...
rnn_unit=5       #Units in hidden layer
stacked_layers=3  #stacked isru layers
batch_size=60     #Batch size
input_size=6      #Input size = double as the input variables of SRU model
...
#——————————————————Define the neural network variables——————————————————
def isru_fw_cell():
    return Improved_SRUCell(rnn_unit)

def isru_bw_cell():
    return Improved_SRUCell(rnn_unit)


def isru(batch):      #Formal parameter：Number of batches entered network
    w_in=weights['in']
    b_in=biases['in']
...
#Tenor needs to be converted into three-dimensional to conduct computation, and the calculated results are used as input of the isru cell.
    #print(input_rnn)

    #layer stack
    stacked_isru_fw=tf.contrib.rnn.MultiRNNCell([isru_fw_cell() for _ in range(stacked_layers)])  #forward sru layer
    stacked_isru_bw=tf.contrib.rnn.MultiRNNCell([isru_bw_cell() for _ in range(stacked_layers)])  #backward sru layer

    #cell=Improved_SRUCell(rnn_unit)
    #init_state=cell.zero_state(batch_size,dtype=tf.float32)

    
    output_rnn,final_states=tf.nn.bidirectional_dynamic_rnn(stacked_isru_fw,stacked_isru_bw,input_rnn,dtype=tf.float32)
    
    #,initial_state=init_state
    #tf.nn.dynamic_rnn(cell, input_rnn,initial_state=init_state, dtype=tf.float32)  
    
    
   #Output_rnn is used to record the result of each output node of isru, and final_states is the result of the last cell.   
   ...
   #Loss function
    ...
    #Set the training times
        ...
        for i in range(300):
            ...
            if step%10==0:
                print(i,loss_)
                print("save the isru model into",saver.save(sess,'model/maestock.model'))
            step+=1
            time_end = time.time()
...
dataframe.to_csv("../isruloss-5-300-mae.csv",index=False,sep=',')
timeframe=pd.DataFrame({'timeperstep':timeperstep,'timeall':alltime})
timeframe.to_csv("../isrutime-5-300-mae.csv",index=False,sep=',')
plt.show()
...
#————————————————Prediction————————————————————
def prediction():
    pred,_=isru(1)       #Predict one by one
    saver=tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        #Recover parameters
...
#Each time we get the prediction results of last time step, we add them up with the previous data to form a new test sample.
        ...
        #plt.figure()
        #plt.plot( normalize_data[-1], color='black',label="realdatabefore")
        #plt.plot( predict, color='r',label="predict")
        #plt.plot(timeperstep1, color='b',label="realpredict")
        #plt.legend(loc=0)'''
        dataframe = pd.DataFrame({'predict': predict})
        dataframe.to_csv("../isrupre-5-300-mae.csv", index=False, sep=',')
        #print(predict)
        plt.show()
