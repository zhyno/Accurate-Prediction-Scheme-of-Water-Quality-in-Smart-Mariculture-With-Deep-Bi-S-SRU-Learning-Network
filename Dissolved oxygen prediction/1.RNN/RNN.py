...
def rnn(batch):      #Formal parameter：Number of batches entered network
    w_in=weights['in']
    b_in=biases['in']
...
#Tenor needs to be converted into three-dimensional to conduct computation, and the calculated results are used as input of the rnn cell.
    #print(input_rnn)
...
#Output_rnn is used to record the result of each output node of rnn, and final_states is the result of the last cell.
...
def train_rnn():
    ...
    with tf.Session() as sess:
        ...
        for i in range(300):
            ...
            if step%10==0:
                print(i,loss_)
                print("save the rnn model into",saver.save(sess,'model/maestock.model'))
            step+=1
            time_end = time.time()
...
dataframe.to_csv("../rnnloss-5-300-mae.csv",index=False,sep=',')
timeframe=pd.DataFrame({'timeperstep':timeperstep,'timeall':alltime})
timeframe.to_csv("../rnntime-5-300-mae.csv",index=False,sep=',')
plt.show()
print(alltime)
...
#————————————————Prediction————————————————————
def prediction():
    pred,_=rnn(1)      #Predict one by one
    saver=tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        #Recover parameters
...
#Each time we get the prediction results of last time step, we add them up with the previous data to form a new test sample.
        ...
        dataframe.to_csv("../rnnpre-5-300-mae.csv", index=False, sep=',')
        #print(predict)
        plt.show()
