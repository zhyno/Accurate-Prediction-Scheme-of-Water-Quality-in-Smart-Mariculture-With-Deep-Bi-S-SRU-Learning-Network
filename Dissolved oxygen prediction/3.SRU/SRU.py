...
def sru(batch):      #Formal parameter：Number of batches entered network
    w_in=weights['in']
    b_in=biases['in']
...
#Tenor needs to be converted into three-dimensional to conduct computation, and the calculated results are used as input of the sru cell.
    #print(input_rnn)
    
    cell=SRUCell(rnn_unit)
    init_state=cell.zero_state(batch_size,dtype=tf.float32)

    output_rnn,final_states=tf.nn.dynamic_rnn(cell, input_rnn,initial_state=init_state, dtype=tf.float32)  
    #Output_rnn is used to record the result of each output node of sru, and final_states is the result of the last cell.
   ...
   #Loss function
    ...
    #Set the training times
        ...
        for i in range(300):
            ...
            if step%10==0:
                print(i,loss_)
                print("save the sru model into",saver.save(sess,'model/maestock.model'))
            step+=1
            time_end = time.time()
...
dataframe.to_csv("../sruloss-5-300-mae.csv",index=False,sep=',')
timeframe=pd.DataFrame({'timeperstep':timeperstep,'timeall':alltime})
timeframe.to_csv("../srutime-5-300-mae.csv",index=False,sep=',')
plt.show()
...
#————————————————Prediction————————————————————
def prediction():
    pred,_=sru(1)       #Predict one by one
    saver=tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        #Recover parameters
...
#Each time we get the prediction results of last time step, we add them up with the previous data to form a new test sample.
        ...
        dataframe.to_csv("../srupre-5-300-mae.csv", index=False, sep=',')
        #print(predict)
        plt.show()
